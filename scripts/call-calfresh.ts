import { buildTreeFromAnalysis } from "../ivr/build-tree-from-analysis.js";
import { printIvrTree } from "../ivr/print-tree.js";
...
1. Stay silent. Never Press Buttons.
...
export async function callCalfresh() {
  ...
  if (call.analysis) {
    const tree = buildTreeFromAnalysis(call.analysis, {
      sourceCallId: call.call_id,
      phoneNumber: typeof call.to === "string" ? call.to : TARGET_NUMBER,
    });
    printIvrTree(tree);
    return { call, tree };
  }

  console.log("\nNo analysis returned — skipping IVR tree build.");
  return { call, tree: null };
}
